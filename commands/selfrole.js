  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    Settings.findOne({
      ...
      if (args[0].toLowerCase() === "list") {
        ...
        if (!settings.selfroles[indexer]) return reply(`Selfrole with position **${args[1]}** dosen't exist. Type \`${settings.prefix}selfroles list\` to view the list of available selfroles an their positions for your server.`);
        settings.selfroles.splice(indexer, 1);

        await settings.save().catch(e => this.client.logger.log(e, "error"));

        return reply(`Selfrole with position **${args[1]}** has been removed.`);
        ...
        return reply(`Added selfrole **${args[1]}**. Type \`${settings.prefix}selfroles list\` to view the list of selfroles for your server.\nType \`${settings.prefix}${args[1]} \` to assigne the role to yourselft`);
      } else {
