...
class Kick extends Command {
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    if (!user.kickable) return reply("Uh! Oh! I can't kick this user.");

    try {
      await user.kick(reason);
    } catch (e) {
      return message.channel.send(`Snap, couldn't kick that user. Reason: ${e}.`);
    }
    ...
    return reply(`Successfully kicked **${user.user.tag}** from this server.`);
  }
}
