...
class Ban extends Command {
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    if (!user) return reply("<a:aRedTick:556121032507916290> Please specify a user to ban.");
    if (!reason) reason = "None Provided";

    if (user.id === message.author.id) return reply("<a:aRedTick:556121032507916290> I'm so sorry... I can't allow self-harm!");
    if (!user.bannable) return reply("<a:aRedTick:556121032507916290> Uh! Oh! I can't ban this user. Looks like he's more powerful than I am!");

    try {
      user.ban(reason);
    } catch (e) {
      return message.channel.send(`<a:aRedTick:556121032507916290> Oh snap! I couldn't ban the specified user! Reason: ${e}.`);
    }
    ...
    return reply(`<a:aGreenTick:556121203136528388> Successfully banned **${user.user.tag}** from this server.`);
  }
}
